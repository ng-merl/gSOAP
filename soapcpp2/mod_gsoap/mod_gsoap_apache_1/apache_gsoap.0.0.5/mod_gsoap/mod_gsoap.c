/** Apache SOAP module for Foto Upload
 * @file mod_gsoap.c
...
#define GSOAP_ID "Apache mod_gsoap gsoap httpd extension 0.0.5"
...
static pool *gsoapConfiguration_getModulePool() {
...
*/
static void SoapSharedLibrary_init(SoapSharedLibrary *This, pool *p, const SoapSharedLibrary *pLib) {
    This->m_pPool = p;
    This->m_hLibrary = NULL;
    This->m_pszPath = ap_pstrdup(p, pLib->m_pszPath);
    This->m_bIsSOAPLibrary = pLib->m_bIsSOAPLibrary;
}
static void SoapSharedLibrary_init2(SoapSharedLibrary *This, pool *p, const char *pszPath) {
    This->m_pPool = p;
    This->m_hLibrary = NULL;
    This->m_pszPath = ap_pstrdup(p, pszPath);
    This->m_bIsSOAPLibrary = false;
}
static void SoapSharedLibrary_clear(SoapSharedLibrary *This, pool *p) {
    This->m_pPool = p;
    This->m_hLibrary = NULL;
    This->m_pszPath = NULL;
    This->m_bIsSOAPLibrary = false;
}
static SoapSharedLibrary *SoapSharedLibrary_create(pool *p) {
    SoapSharedLibrary *This = (SoapSharedLibrary *)ap_pcalloc(p, sizeof(SoapSharedLibrary));
 ...
 */
static const char *SoapSharedLibrary_load(SoapSharedLibrary *This, pool *pTempPool) {
    const char *pszError = NULL;
    const int nFlags = RTLD_LAZY | RTLD_GLOBAL;
    This->m_hLibrary = (void *)dlopen(This->m_pszPath, nFlags);
    pszError = dlerror();
    if (NULL == This->m_hLibrary) {
		pszError = ap_psprintf(NULL == pTempPool ? This->m_pPool : pTempPool, "mod_gsoap: %s loading library %s", pszError, This->m_pszPath);
    }
    return pszError;
}
/**
...
/*-------------------------------------------------------*/
static void SoapSharedLibraries_init(SoapSharedLibraries *This, pool *p) {
    This->m_pPool = p;
    ...
    This->m_pIntf = (struct apache_soap_interface *)ap_pcalloc(p, sizeof(struct apache_soap_interface));
}
...
static void SoapSharedLibraries_addLibrary(SoapSharedLibraries *This, SoapSharedLibrary *pLibrary) {
    ...
    if (!SoapSharedLibraries_contains(This, pLibrary->m_pszPath)) {
		SoapSharedLibrary **ppNewLib = (SoapSharedLibrary **)ap_push_array(This->m_pLibraries);
		*ppNewLib = pLibrary;
...
static const char *SoapSharedLibraries_getEntryPoints(SoapSharedLibraries *This, SoapSharedLibrary *pLib, pool *pTempPool) {
 ...
 */
static const char *SoapSharedLibraries_loadAllLibraries(SoapSharedLibraries *This, pool *pTempPool) {
...
static void gsoapConfiguration_init(gsoapConfiguration *This, pool *p) {
    This->m_pLibraries = (SoapSharedLibraries *)ap_pcalloc(p, sizeof(SoapSharedLibraries));
    SoapSharedLibraries_init(This->m_pLibraries, p);
    This->m_Type = ct_directory;
}
static gsoapConfiguration *gsoapConfiguration_create(pool *p) {
    gsoapConfiguration *pConfig = (gsoapConfiguration *)ap_pcalloc(p, sizeof(gsoapConfiguration));
...
static void SendErrorMessage(request_rec *r, const char *pszError) {
    ...
    ap_log_error(APLOG_MARK, APLOG_ERR, r->server, "mod_gsoap: %s", pszError, NULL);
...
static size_t frecv(struct soap *psoap, char *pBuf, size_t len) {
    request_rec *r = NULL;
    int nRet = 0;
...
static int fsend(struct soap *psoap, const char *pBuf, size_t len) {
	int nWritten = 0;
	int nRet = SOAP_OK;
    gsoapRequestConfiguration *pRqConf = getRequestConfiguration(psoap);
    if (NULL != pRqConf) {
...
static int gsoap_handler(request_rec *r) {
    ...
    if (NULL == pszError) {
		...
		pRqConf->m_pszAllHeaders = ap_pcalloc(r->pool, pRqConf->m_nHeaderLength + 1);
 ...
 */
static void *gsoap_merge_dir_config(pool *p, void *parent_conf,void *newloc_conf) {
...
static gsoapRequestConfiguration *getRequestConfiguration(struct soap *soap) {
	gsoapRequestConfiguration *pRqConf = (gsoapRequestConfiguration *)soap->fplugin(soap, GSOAP_ID);
	return pRqConf;
}
