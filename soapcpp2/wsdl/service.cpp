	...
	{ // /definitions/portType/operation/ associated with /definitions/binding/operation
	  ...
	  { wsdl__input *input = wsdl__operation_->input;
	    ...
	    { // MUST have an input, otherwise can't generate a service operation
		  ...
		  fprintf(stderr, "Error: no SOAP RPC operation namespace\n");
	      }
	      else
...
{ // compile the definitions and generate gSOAP header file
  ...
  fprintf(stream, "/*  Modify this file to customize the generated data type declarations */\n\n");
  if (!lflag)
  ...
  { fprintf(stream, "\n/*\nTo customize the names of the namespace prefixes generated by wsdl2h, modify\nthe prefix names below and add the modified lines to typemap.dat to run wsdl2h:\n\n");
    ...
    { for (vector<xs__complexType>::const_iterator complexType = (*schema3)->complexType.begin(); complexType != (*schema3)->complexType.end(); ++complexType)
      ...
      for (vector<xs__element>::const_iterator element = (*schema3)->element.begin(); element != (*schema3)->element.end(); ++element)
...
{ fprintf(stream, "\n/* Service %s operations:\n\n", name);
  ...
  { if (*op2 && (*op2)->input)
    { fprintf(stream, "\n/* Service %s operation %s\n\n  C stub function (defined in soapClient.c[pp]):\n", name, (*op2)->input_name);
      ...
      { if ((*op2)->style == document && (*op2)->output->message && (*op2)->output->message->part.size() == 1)
...
{ bool anonymous = parameterOrder != NULL;
  ...
  if (output_name)
  { if (style == document && output->message && output->message->part.size() == 1)
    { // Shortcut: do not generate wrapper struct
