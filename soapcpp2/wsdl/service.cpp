...
{ // compile the definitions and generate gSOAP header file
  ...
  { const char *s, *t = types.cname("_", NULL, *j);
    ...
    { if (!mflag)
      { if (**j == '"')
	  fprintf(stream, "\n// Imported element %s defined by %s\n", *j, t);
        else
	{ fprintf(stream, "\n/// Built-in element \"%s\"\n", *j);
  ...
  { const char *s, *t = types.cname(NULL, NULL, *k);
    ...
    { if (mflag)
      { if (**k == '"')
	  fprintf(stream, "\n/// Imported attribute %s\n", *k);
        else
	  fprintf(stream, "\n/// Built-in attribute \"%s\"\n", *k);
      }
      else
        fprintf(stream, "\n// base.h must define attribute: ");
      fprintf(stream, "%s\n", s);
      s = types.usetypemap[t];
      if (s && *s && types.knames.find(s) == types.knames.end())
        types.knames.insert(s);
    ...
    { s = types.tname(NULL, NULL, "xsd:string");
      if (!mflag)
      { if (**k == '"')
          fprintf(stream, "\n// Imported attribute %s defined by %s\n", *k, t);
        else
        { fprintf(stream, "\n/// Built-in attribute \"%s\"\n", *k);
          fprintf(stream, "typedef %s %s;\n", s, t);
        }
  ...
  { banner("Services");
    ...
    if (definitions.documentation)
    { section(definitions.name?definitions.name:"Service", "_documentation Documentation", NULL);
      text(definitions.documentation);
    }
    if (definitions.types && definitions.types->documentation)
    { section(definitions.name?definitions.name:"Service", "_types Types", NULL);
      text(definitions.types->documentation);
    }
    section(definitions.name?definitions.name:"Service", "_bindings Bindings", NULL);
    ...
    { Service *sv = (*service3).second;
      ...
      { fprintf(stream, "\n/**\n");
        ...
        { section((*service_doc).first, "_service Service Documentation", (*service_doc).first);
        ...
        { section((*port_doc).first, "_port Port Documentation", (*port_doc).first);
        ...
        { section((*binding_doc).first, "_binding Binding Documentation", (*binding_doc).first);
