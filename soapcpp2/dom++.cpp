/*	dom++.cpp
	...
	The internals of the C++ DOM parser is based on the gSOAP C DOM parser
	(dom.c).
...
void
SOAP_FMAC2
soap_mark_xsd__anyType(struct soap *soap, soap_dom_element const *node)
{ if (node)
  { if (node->__type && node->node)
      soap_markelement(soap, node->node, node->__type);
    else if (!node->data && !node->wide)
      for (soap_dom_element *elt = node->elts; elt; elt = elt->next)
...
void
SOAP_FMAC2
soap_default_xsd__anyType(struct soap *soap, soap_dom_element *node)
{ node->next = NULL;
  node->prnt = NULL;
  node->nstr = NULL;
  node->name = NULL;
...
static int out_element(struct soap *soap, soap_dom_element const *node, const char *prefix, const char *name, const char *nstr)
{ if (!prefix)
  { if (node->__type && node->node)
...
int
SOAP_FMAC2
soap_out_xsd__anyType(struct soap *soap, const char *tag, int id, soap_dom_element const *node, const char *type)
{ if (node)
  { register struct soap_ilist *p = NULL, *q;
    ...
    if (node->__type && node->node)
      return SOAP_OK;
    for (soap_dom_attribute *att = node->atts; att; att = att->next)
    { if (att->name)
      { if (att->nstr)
...
soap_dom_element *
SOAP_FMAC2
soap_in_xsd__anyType(struct soap *soap, const char *tag, soap_dom_element *node, const char *type)
{ register struct soap_attribute *tp;
  register soap_dom_attribute **att;
  register struct soap_nlist *np;
  register char *s;
  if (soap_peek_element(soap))
    return NULL;
  if (!node)
    if (!(node = soap_new_xsd__anyType(soap, -1)))
    { soap->error = SOAP_EOM;
      return NULL;
    }
  node->next = NULL;
  ...
  { if ((node->node = soap_getelement(soap, &node->__type)))
    ...
    { strcpy(soap->type, soap->tag);
      if ((node->node = soap_getelement(soap, &node->__type)))
      { DBGLOG(TEST, SOAP_MESSAGE(fdebug, "DOM node contains type %d from element %s\n", node->__type, soap->tag));
        return node;
      }
    }
  }
  ...
  for (tp = soap->attributes; tp; tp = tp->next)
    ...
    { np = soap->nlist;
      ...
      *att = soap_new_xsd__anyTypeAttribute(soap, -1);
      if (!*att)
      { soap->error = SOAP_EOM;
        return NULL;
      }
...
void
SOAP_FMAC2
soap_mark_xsd__anyTypeAttribute(struct soap *soap, soap_dom_attribute const *a)
{ }

SOAP_FMAC1
soap_dom_attribute *
SOAP_FMAC2
soap_in_xsd__anyTypeAttribute(struct soap *soap, char const *tag, soap_dom_attribute *a, char const *type)
{ return NULL;
}
...
int
SOAP_FMAC2
soap_out_xsd__anyTypeAttribute(struct soap *soap, char const *tag, int id, soap_dom_attribute const *a, char const *type)
{ return SOAP_EOF;
}
...
{ elt->prnt = this;
  for (soap_dom_element *e = elts; e; e = e->next)
    if (!e->next)
    { e->next = elt;
      return *this;
    }
...
{ for (soap_dom_attribute *a = atts; a; a = a->next)
    if (!a->next)
    { a->next = att;
      return *this;
    }
...
soap_dom_iterator::soap_dom_iterator(soap_dom_element *elt)
{ this->elt = elt;
  nstr = NULL;
  name = NULL;
  type = 0;
...
soap_dom_element &soap_dom_iterator::operator*() const
{ return *this->elt;
}
...
ostream &operator<<(ostream &o, const soap_dom_element &e)
{ if (!e.soap)
  { struct soap soap;
...
istream &operator>>(istream &i, soap_dom_element &e)
{ if (!e.soap)
    e.soap = soap_new();
