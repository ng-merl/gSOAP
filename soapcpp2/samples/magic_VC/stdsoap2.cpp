...
#ifdef __cplusplus
SOAP_SOURCE_STAMP("@(#) stdsoap2.cpp ver 2.2 2002-12-12 12:00:00 GMT")
extern "C" {
#else
SOAP_SOURCE_STAMP("@(#) stdsoap2.c ver 2.2 2002-12-12 12:00:00 GMT")
#endif
...
static void soap_add_ptrs(struct soap*, char*, char*, long);
...
{ if ((soap->mode & SOAP_IO) == SOAP_IO_STORE)
  ...
  DBGMSG(SENT, s, n);
  if ((soap->error = soap->fsend(soap, s, n)))
    return soap->error;
  if ((soap->mode & SOAP_IO) == SOAP_IO_CHUNK)
  { DBGMSG(SENT, "\r\n", 2);
...
{ if (soap->bufidx)
  {
    ...
    { soap->d_stream.next_in = (Byte*)soap->buf;
      ...
      do
      { soap->d_stream.next_out = (Byte*)soap->d_buf;
        ...
        { if (soap_flush_raw(soap, soap->d_buf, SOAP_BUFLEN - soap->d_stream.avail_out))
            return soap->error;
        }
      } while (soap->d_stream.avail_in || !soap->d_stream.avail_out);
    }
    else
#endif
...
zlib_again:
    ...
    { memcpy(soap->buf, soap->d_buf, SOAP_BUFLEN);
      soap->buflen = soap->d_buflen;
    }
  }
#endif
  ...
  { if (soap->chunksize > 0)
    ...
    { char tmp[8], *t;
      ...
      DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Getting chunk size from buffered HTTP (%u %u)\n", (unsigned int)soap->bufidx, (unsigned int)soap->buflen));
      ...
      *t = '\0';
      ...
      { soap->buflen = soap->chunkbuflen;
        ...
        soap->chunkbuflen = 0;
        DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Passed end of buffer for chunked HTTP\n"));
      }
  ...
  DBGMSG(RECV, soap->buf + soap->bufidx, ret);
#ifdef WITH_ZLIB
  if (soap->mode & SOAP_ENC_ZLIB)
  { int r;
    memcpy(soap->d_buf, soap->buf, SOAP_BUFLEN);
    soap->d_stream.next_in = (Byte*)(soap->d_buf + soap->bufidx);
    soap->d_stream.avail_in = (unsigned int)ret;
    soap->d_stream.next_out = (Byte*)soap->buf;
    soap->d_stream.avail_out = SOAP_BUFLEN;
    DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Attempt to inflate %u bytes\n", (unsigned int)ret));
    r = inflate(&soap->d_stream, Z_NO_FLUSH);
    if (r == Z_OK || r == Z_STREAM_END)
    { soap->bufidx = 0;
      soap->d_buflen = soap->buflen;
...
{ int i;
  ...
  DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Pointers updated\n"));
}

/******************************************************************************/
static void
soap_add_ptrs(struct soap *soap, char *start, char *end, long offset)
{ int i;
  struct soap_ilist *ip;
  register char *p;
  for (i = 0; i < SOAP_IDHASH; i++)
    for (ip = soap->iht[i]; ip; ip = ip->next)
      { for (p = (char*)ip->link; p; p = *(char**)p)
        { if (p >= start && p < end)
          { DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Link added id='%s' %p to %p\n", ip->id, p, p+offset));
            *(void**)(p+offset) = ip->link;
            ip->link = (void*)(p+offset);
          }
        }
        for (p = (char*)ip->copy; p; p = *(char**)p)
        { if (p >= start && p < end)
          { DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Copy added id='%s' %p to %p\n", ip->id, p, p+offset));
            *(void**)(p+offset) = ip->copy;
            ip->copy = (void*)(p+offset);
          }
        }
      }
  DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Pointers added\n"));
}
...
{ void *p, **q;
  ...
  ip->copy = NULL;
  while (q) 
  { DBGLOG(TEST, SOAP_MESSAGE(fdebug, "... step ... copy %p -> %p (%u bytes)\n", ip->ptr, q, (unsigned int)ip->size));
    p = *q;
    soap_add_ptrs(soap, (char*)ip->ptr, (char*)ip->ptr + ip->size, (char*)q - (char*)ip->ptr);
...
{ int i;
  struct soap_nlist *np = (struct soap_nlist*)malloc(sizeof(struct soap_nlist) + strlen(id));
  if (!np)
    return soap->error = SOAP_EOM;
  np->next = soap->nlist;
  soap->nlist = np;
      ...
      { if (soap->namespaces[i].out)
        ...
        { if (!strcmp(ns, soap_env1))
          { soap->version = 1; /* and make sure we use SOAP 1.1 encoding */
            if (soap->namespaces[1].out)
              free(soap->namespaces[1].out);
            if ((soap->namespaces[1].out = (char*)malloc(strlen(soap_enc1)+1)))
              strcpy(soap->namespaces[1].out, soap_enc1);
          }
          else if (!strcmp(ns, soap_env2))
          { soap->version = 2; /* and make sure we use the SOAP 1.2 encoding */
            if (soap->namespaces[1].out)
              free(soap->namespaces[1].out);
            if ((soap->namespaces[1].out = (char*)malloc(strlen(soap_enc2)+1)))
              strcpy(soap->namespaces[1].out, soap_enc2);
          }
        }
        break;
      }
...
{ struct soap_nlist *np = soap->nlist;
  while (np && (np->id[n1] || strncmp(np->id, id1, n1)))
    np = np->next;
  if (np)
...
{ ssl_init();
  ...
  { RAND_seed(soap->buf, SOAP_BUFLEN);
    while (!RAND_status())
    { int r;
#ifdef HAVE_RAND_R
      unsigned int s;
  ...
  { if (connect(soap->socket, (struct sockaddr*)&sockaddr, sizeof(sockaddr)))
    { if (soap->connect_timeout && (soap_socket_errno == SOAP_EINPROGRESS || soap_socket_errno == SOAP_EWOULDBLOCK))
      { struct timeval timeout;
        ...
        socklen_t n = sizeof(struct sockaddr_in);
#elif defined(WIN32) || defined(__APPLE__) || defined(HP_UX)
...
{ soap_clr_attr(soap);
  ...
  { soap->mode = soap->omode;
    soap->mode |= SOAP_IO_LENGTH | SOAP_ENC_DIME;
  }
  else
  { soap->mode = soap->omode;
...
{ int i;
  ...
  { soap_array_pointer_enter(soap, p, a, t, &pp);
    pp->mark1 = 0;
    pp->mark2 = 0;
  }
...
{ DBGLOG(TEST, SOAP_MESSAGE(fdebug, "End send\n"));
  ...
  { if (soap_flush(soap))
    ...
    { char *p;
      ...
      { soap->mode--;
        if (soap->status >= 0)
          ...
          soap->error = soap->fpost(soap, soap->endpoint, soap->host, soap->path, soap->action, soap->blist->size);
    ...
    { DBGMSG(SENT, "0\r\n\r\n", 5);
      if ((soap->error = soap->fsend(soap, "0\r\n\r\n", 5)))
        return soap->error;
    }
  }
...
{ soap->version = 1; /* default SOAP 1.1 */
  ...
  *soap->path = '\0';
  ...
  soap->z_level = Z_DEFAULT_COMPRESSION;
#endif
  soap_init_logs(soap);
  soap_init_iht(soap);
  soap_init_pht(soap);
...
{ DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Initializing\n"));
  ...
  *soap->path = '\0';
  ...
  { soap->namespaces[0].id = "SOAP-ENV"; /* override to make sure */
    ...
    if (soap->namespaces[0].ns)
    { if (!strcmp(soap->namespaces[0].ns, soap_env1))
        soap->version = 1;
      else if (!strcmp(soap->namespaces[0].ns, soap_env2))
        soap->version = 2;
    }
  }
  soap->dime_chunksize = 0;
...
{ struct soap_attribute *tp;
  ...
  if (!soap->ns)
  { struct Namespace *ns = soap->namespaces;
...
{ const char *t;
  ...
  { switch (c)
    ...
    case '\'':
      ...
      break;
    default:
      if (c >= 32 || (c < 0 && (soap->mode & SOAP_C_UTFSTRING)))
      { tmp[0] = c;
        tmp[1] = '\0';
      }
      else
...
{
#ifdef WIN32
  sprintf(soap->tmpbuf, "%I64d", n);
#else
  sprintf(soap->tmpbuf, "%lld", n);
#endif
  return soap->tmpbuf;
}
...
{
#ifdef WIN32
  sprintf(soap->tmpbuf, "%I64u", n);
#else
  sprintf(soap->tmpbuf, "%llu", n);
#endif
  return soap->tmpbuf;
}
...
{ if (s)
  { const char *p = strchr(s, ':');
    ...
    { struct soap_nlist *np = soap->nlist;
      ...
      if (np)
      { if (np->index >= 0)
        { const char *q = soap->namespaces[np->index].id;
          if (q)
...
{ struct tm T;
  ...
  { struct timeb t;
    ...
    sprintf(soap->tmpbuf+strlen(soap->tmpbuf), "%+d:%d", t.timezone/60, abs(t.timezone)%60);
#else
...
{ int i;
  ...
  { strncpy(soap->tmpbuf, tag, t-tag);
    soap->tmpbuf[t-tag] = '\0';
    for (i = 0; soap->namespaces[i].id; i++)
      if (!strcmp(soap->tmpbuf, soap->namespaces[i].id))
        break;
    sprintf(soap->tmpbuf, "<%s xmlns=\"%s\">", t+1, soap->namespaces[i].ns ? soap->namespaces[i].ns : "");
  }
  else
  ...
  { while (--i > 0)
    ...
    *s = '\0';
    ...
    if (i+1 == len) /* empty line: end of HTTP header */
      break;
    c = soap_unget(soap, soap_get2(soap));
...
{ if (soap->mode & SOAP_ENC_DIME)
  { size_t count;
    ...
    if (soap->namespaces)
    { if (soap->namespaces[0].out)
        soap->dime_type = (char*)soap->namespaces[0].out;
      else
...
{ if (soap_getdimehdr(soap))
  ...
  { if (!(soap->dime_ptr = soap_getdimefield(soap, soap->dime_size)) && soap->error)
      return soap->error;
  }
  return SOAP_OK;
}
...
/******************************************************************************/
static const char *
soap_getcookiefield(char *buf, int len, const char *val)
{ const char *s;
  char *t;
  for (s = val; *s; s++)
    if (*s != ' ' && *s != '=')
...
{ struct soap_cookie *p = NULL, *q;
  ...
  s = val;
  while (*s)
  { s = soap_getcookiefield(tmp, sizeof(tmp), s);
    if (!soap_tag_cmp(tmp, "$Version"))
    { if ((s = soap_getcookiefield(tmp, sizeof(tmp), s)))
      { if (p)
          p->version = (int)atoi(tmp);
        else
          version = (int)atoi(tmp);
      }
    }
    ...
    { s = soap_getcookiefield(tmp, sizeof(tmp), s);
      p->version = (unsigned int)atoi(tmp);
    }
...
{ struct soap_cookie *p;
  ...
  if (!(s = getenv("HTTP_COOKIE")))
    return SOAP_EOF;
  do
  { s = soap_getcookiefield(key, sizeof(key), s);
    s = soap_getcookiefield(val, sizeof(val), s);
...
#endif
  c = soap_get2(soap);
  if ((c&0xFFFC) == (SOAP_DIME_VERSION|SOAP_DIME_MB) && (soap_unget2(soap, soap_get2(soap))&0xFFF0) == 0x20)
    soap->mode |= SOAP_ENC_DIME;
  else
    while (blank(c))
      c = soap_get2(soap);
...
{ char header[SOAP_HDRLEN], status[SOAP_TAGLEN], *s, *t;
  DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Waiting for response...\n"));
  *soap->endpoint = '\0';
  soap->length = 0;
  do
  { if (soap_getline(soap, status, SOAP_TAGLEN))
      return SOAP_EOF;
    ...
    { if (soap_getline(soap, header, SOAP_HDRLEN))
      ...
      soap->fparsehdr(soap, header, t);
    }
    t = strchr(status, ' ');
  } while (t && !strncmp(t, " 100 ", 5));
  s = strstr(status, "HTTP/");
  ...
  if (s && !strncmp(status, "POST ", 5))
  { int n = strlen(soap->endpoint) + (s-status) - 6;
    if (n >= (int)sizeof(soap->endpoint))
      n = sizeof(soap->endpoint)-1;
    strncat(soap->endpoint, status+5, n-strlen(soap->endpoint));
    soap->endpoint[n] = '\0';
    DBGLOG(TEST,SOAP_MESSAGE(fdebug, "Target endpoint='%s'\n", soap->endpoint));
    return SOAP_OK;
  }
  if (!t || !strncmp(t, " 200", 4) || !strncmp(t, " 400", 4) || !strncmp(t, " 500", 4))
    return SOAP_OK;
  strcpy(soap->msgbuf, status);
...
{ if (!soap_tag_cmp(key, "Host"))
  { strcpy(soap->endpoint, "http://");
    strncat(soap->endpoint, val, sizeof(soap->endpoint)-7);
  ...
  { if (!soap_tag_cmp(val, "gzip*") || !soap_tag_cmp(val, "compress*"))
      soap->mode |= SOAP_ENC_ZLIB;
#ifndef WITH_ZLIB
...
{ if (soap->encodingStyle && !*soap->encodingStyle && soap->namespaces)
  { if (soap->namespaces[1].out)
      soap->encodingStyle = soap->namespaces[1].out;
    else
      soap->encodingStyle = soap->namespaces[1].ns;
  }
  if (soap->encodingStyle)
  { sprintf(soap->tmpbuf, "<SOAP-ENV:Body SOAP-ENV:encodingStyle=\"%s\">", soap->encodingStyle);
    return soap_send(soap, soap->tmpbuf);
  }
...
{ char host[SOAP_TAGLEN];
  ...
  { if (soap->socket < 0 || strcmp(soap->host, host) || soap->port != port)
    ...
    { struct timeval timeout;
      ...
      FD_ZERO(&fd);
      FD_SET(soap->socket, &fd);
      if (select(soap->socket + 1, &fd, &fd, &fd, &timeout) < 0)
      { soap->keep_alive = 0; /* force close */
        soap_closesock(soap);
  ...
  { int n = soap->mode;
    soap->mode &= ~(SOAP_IO_CHUNK | SOAP_ENC_ZLIB);
...
{ if (soap->mode & SOAP_ENC_DIME)
    soap->error = soap->fposthdr(soap, "Content-Type", "application/dime; charset=us-ascii");
  else if (soap->version == 2)
    soap->error = soap->fposthdr(soap, "Content-Type", "application/soap+xml; charset=utf-8");
  else
    soap->error = soap->fposthdr(soap, "Content-Type", "text/xml; charset=utf-8");
  if (soap->error)
    return soap->error;
#ifdef WITH_ZLIB
...
{ if (!endpoint || (strncmp(endpoint, "http:", 5) && strncmp(endpoint, "https:", 6)))
    return SOAP_OK;
  if (soap->proxy_host)
    sprintf(soap->tmpbuf, "POST %s HTTP/%s", endpoint, soap->http_version);
  else
    sprintf(soap->tmpbuf, "POST /%s HTTP/%s", path, soap->http_version);
  if ((soap->error = soap->fposthdr(soap, soap->tmpbuf, NULL))
   || (soap->error = soap->fposthdr(soap, "Host", host))
   || (soap->error = soap->fposthdr(soap, "User-Agent", "gSOAP/2.2"))
   || (soap->error = soap_puthttphdr(soap, count)))
    return soap->error;
    ...
    case SOAP_ZLIB_ERROR:
      ...
      *s = "Zlib error";
      if (d)
        *d = soap->d_stream.msg;
#else
      *s = "Zlib not installed for required (de)compression";
#endif
      break;
    case SOAP_EOD:
      *s = "End of DIME error";
...
{ int soap_error = soap->error;
  ...
  if (soap_response(soap, soap_error)
   || soap_envelope_begin_out(soap)
   || soap_putheader(soap)
   || soap_body_begin_out(soap)
   || soap_putfault(soap)
