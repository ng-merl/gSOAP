...
{ Entry *p, *q;
  ...
  { Data *d;
    if (sp2->data && sp2->URI)
      fprintf(fd, "\nSchema %s types:\n\n", sp2->URI);
    for (d = sp2->data; d; d = d->next)
      fprintf(fd, "%s\n\t%s\n", ns_add(d->name, sp2->ns), d->part);
  }
  fprintf(fd, "\n </documentation>\n\n");
  for (s = nslist; s; s = s->next)
  ...
  { for (p = t->list; p; p = p->next)
    ...
    if (is_document(style))
      if (sp && sp->port)
        ...
        fprintf(fd, "type=\"tns:%sPortType\">\n <SOAP:binding style=\"document\"", name);
    else
      if (sp && sp->port)
        fprintf(fd, "type=\"tns:%s\">\n <SOAP:binding style=\"rpc\"", sp->port);
      else
        fprintf(fd, "type=\"tns:%sPortType\">\n <SOAP:binding style=\"rpc\"", name);
    if (sp && sp->transport)
...
{ Entry *method;
  ...
  fprintf(fd, "\n\nSOAP_BEGIN_NAMESPACE(%s)", prefix);
  fprintf(fd, "\n\n%s::%s()\n{\t%s(SOAP_IO_DEFAULT, SOAP_IO_DEFAULT);\n}", name, name, name);
...
{ int i=1;
  ...
  for (p = table->list; p != (Entry*) 0; p = p->next)
	...
	gen_call_method(fclient, table, p, NULL); /* generate_call(table, p); */
  ...
  for (p = table->list; p != (Entry*) 0; p = p->next)
    if (p->info.typ->type==Tfun && !(p->info.sto & Sextern) && !is_imported(p->info.typ))
	gen_serve_method(fserver, table, p, NULL); /* generate_server(table, p); */
}
...
{ char *s, *t;
  ...
  if (suffix)
  { s = emalloc(strlen(t) + strlen(suffix) + 1);
    strcpy(s, t);
    strcat(s, suffix);
    t = s;
  }
  if (is_keyword(t))
  { s = emalloc(strlen(t) + 2);
    strcpy(s, t);
    strcat(s, "_");
    return s;
  }
...
{ Entry *p;
  ...
  fprintf(fout,"\n\t\t{\tsoap_peek_element(soap);\n\t\t\tif (soap->position)\n\t\t\t{\ti = soap->positions[0];\n\t\t\t\tif (i < 0 || i >= %d)\n\t\t\t\t{\tsoap->error = SOAP_IOB;\n\t\t\t\t\treturn NULL;\n\t\t\t\t}\n\t\t\t}", get_dimension(typ));
	...
	{ fprintf(fout, "const struct soap_code_map *map;");
          t = (Table*)typ->ref;
          if (t->list && strstr(ns_remove1(t->list->sym->name), "__"))
	    fprintf(fout, "\n\tchar *t;");
	  fprintf(fout, "\n\tif (!s)\n\t\treturn SOAP_OK;");
          if (t->list && strstr(ns_remove1(t->list->sym->name), "__"))
