...
{	Entry *p;
	...
	if (cflag)
		semwarn("Option -c conflicts with the use of class declarations in header file input");
...
{ Entry *p, *q;
  ...
  if (typetable)
    ...
    { if (is_external(p->info.typ))
        fprintf(fheader, "%s ", c_type(p->info.typ));
    }
  fprintf(fheader, "*/\n\n/* Enumerations */");
  fflush(fheader);
  if (enumtable)
    ...
    { fprintf(fheader, "\n");
      ...
      { q = ((Table*)p->info.typ->ref)->list;
        ...
        { 
#ifdef WIN32
	  fprintf(fheader, "%s = %I64d", q->sym->name, q->info.val.i);
#else
...
{ static char buf[1024];
  ...
  if (e->info.hasval)
    ...
    { case Tchar:
      ...
      case Tullong:
#ifdef WIN32
        sprintf(buf, " default=\"%I64d\"", e->info.val.i);
#else
        sprintf(buf, " default=\"%lld\"", e->info.val.i);
#endif
	break;
      case Tfloat:
      case Tdouble:
  ...
  for (r = table->list; r; r = r->next)
    ...
    { p = entry(table, r->sym);
      ...
      if (is_transient(q->info.typ))
        fprintf(fd,") { return soap ? soap_send_%s(soap, endpoint, ", r->sym->name);
      else
        fprintf(fd,", %s%s) { return soap ? soap_call_%s(soap, endpoint, ", c_storage(q->info.sto), c_type_id(q->info.typ, q->sym->name), r->sym->name);
      m = NULL;
...
{ Entry *q;
  char *s, tmp[32];
  int i, d;
  if (!is_transient(p->info.typ) && p->info.typ->type != Tfun && strncmp(p->sym->name, "__size", 6))
	...
	{ d = get_Darraydims(p->info.typ);
      ...
      { case Tchar:
        ...
        case Tullong:
          if (p->info.hasval)
#ifdef WIN32
	    fprintf(fd, "%I64d", p->info.val.i);
#else
	    fprintf(fd, "%lld", p->info.val.i);
#endif
	  else
	    fprintf(fd, "0");
	  break;
...
{ static char buf[1024];
  ...
  if (e->info.hasval)
	...
	break;
      case Tenum:
#ifdef WIN32
        sprintf(buf, " = (%s)%I64d", c_type(e->info.typ), e->info.val.i);
#else
        sprintf(buf, " = (%s)%lld", c_type(e->info.typ), e->info.val.i);
#endif
	break;
      default:
	...
	{ if (p->info.sto & Sattribute)
	    if (is_string(p->info.typ))
	      fprintf(fout, "\n\tsoap_set_attr(soap, \"%s\", a->%s);", ns_convert(p->sym->name), p->sym->name);
            else if (p->info.typ->type == Tllong || p->info.typ->type == Tullong || p->info.typ->type == Tenum)
	...
	*/
    ...
    case Tenum:
      ...
      { fprintf(fhead, "\n\nSOAP_FMAC1 const char* SOAP_FMAC2 soap_%s2s(struct soap*, %s);", c_ident(typ), c_type(typ));
        ...
        fprintf(fout, "\n\t}\n#ifdef WIN32");
        fprintf(fout, "\n\tsprintf(soap->tmpbuf, \"%%I64d\", (LONG64)n);");
        fprintf(fout, "\n#else");
        fprintf(fout, "\n\tsprintf(soap->tmpbuf, \"%%lld\", (LONG64)n);");
        fprintf(fout, "\n#endif");
        fprintf(fout, "\n\treturn soap->tmpbuf;");
        fprintf(fout, "\n}");
      }
      ...
      { fprintf(fout, "\n\nSOAP_FMAC1 int SOAP_FMAC2 soap_out_%s(struct soap *soap, const char *tag, int id, const %s, const char *type)", c_ident(typ), c_type_id(typ, "*a"));
        ...
        { for (p = t->list; p; p = p->next)
#ifdef WIN32
	    fprintf(fout, "\n\t\t\tcase %I64d: soap_send(soap, \"%s \"); break;", p->info.val.i, ns_convert(p->sym->name));
#else
  ...
  { case Tstruct:
      ...
      { fprintf(fout, "\n\tif (soap_peek_element(soap))\n\t\treturn NULL;");
        ...
        fprintf(fout,"\n\t\tsoap_default_%s(soap, a);",c_ident(typ));
          ...
          { for (p = t->list; p; p = p->next) 
	      if (p->info.sto & Sattribute)
                if (p->info.typ->type == Tllong || p->info.typ->type == Tullong || p->info.typ->type == Tenum)
